...
#import "AboutUsMetodVC.h"
#import "UsCenterCell.h"
#import "UserPrivacyVC.h"
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    switch (indexPath.row) {
        case 0:{
             UserPrivacyVC *controller = [[UserPrivacyVC  alloc] init];
              controller.hidesBottomBarWhenPushed = YES;
              [self.navigationController pushViewController:controller animated:YES];
           }
            break;
        case 1:{
            FeedbackMetodVC*controller = [[FeedbackMetodVC  alloc] init];
                      controller.hidesBottomBarWhenPushed = YES;
                      [self.navigationController pushViewController:controller animated:YES];
        }
            break;
...
-(void)setClick{
     ...
     LoginMetodVC*loginVC = [LoginMetodVC new];
//     aboutUs.hidesBottomBarWhenPushed = YES;
//     [self.navigationController pushViewController:aboutUs animated:YES];
    [self.navigationController presentViewController:loginVC animated:YES completion:nil];
    
    
//    AboutUsMetodVC*aboutUs = [AboutUsMetodVC new];
//    aboutUs.hidesBottomBarWhenPushed = YES;
//    [self.navigationController pushViewController:aboutUs animated:YES];
}
