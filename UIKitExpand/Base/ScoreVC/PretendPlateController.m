#import "PretendPlateController.h"
#import "MJRefresh.h"
#import "CopyManmadeCatastropheManager.h"
#import "LoanArmy.h"
#import "CheerFunnelThat.h"
#import "RescueDuskController.h"
#import "RenewTowardPhotoView.h"
#import "TableViewAnimationKitHeaders.h"
@interface PretendPlateController ()
@end
@implementation PretendPlateController
- (void)viewDidLoad {
    [super viewDidLoad];
     self.title = @"球赛规则";
     [self.tableView registerNib:[UINib nibWithNibName:@"CheerFunnelThat" bundle:nil] forCellReuseIdentifier:@"Newcell"];
...
- (void)worshipForFundimentalWhisky{
    [[CopyManmadeCatastropheManager sharedManager]tenderLandmine:URL_Gamerule params:@{} completed:^(BOOL ret, id obj) {
        if (ret) {
            ...
            [self.dataSource removeAllObjects];
                 ...
                 NSArray *arr = [obj objectForKey:@"game"];
                      for (NSDictionary *dic in arr) {
                         LoanArmy *model = [LoanArmy setModelWithDictionary:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    CheerFunnelThat *cell = [tableView dequeueReusableCellWithIdentifier:@"Newcell" forIndexPath:indexPath];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    LoanArmy *model = self.dataSource[indexPath.row];
...
- (void)unloadFluentMidst:(UITableView *)tableView {
    [WarnGiantBar showWithAnimationType:XSTableViewAnimationTypeToTop tableView:tableView];
}
@end
