...
#import "SmellLeastCarrotManager.h"
#import "AttemptSinger.h"
#import "ConservativePrejudiceView.h"

@interface SmellLeastCarrotManager ()
...
/**
 足球
 */
@property (nonatomic, strong) ConservativePrejudiceView *football;
...
static SmellLeastCarrotManager *mazeManager = nil;

@implementation SmellLeastCarrotManager

+ (SmellLeastCarrotManager *)manager {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        mazeManager = [[SmellLeastCarrotManager alloc] init];
    });
    return mazeManager;
}
...
// 数据源：懒加载
- (NSMutableArray<AttemptSinger *> *)mazeCells {
    if (!_mazeCells) {
...
- (DotClearHorizonView *)generateMazeWithRows:(NSInteger)rows withCols:(NSInteger)cols withSpace:(NSInteger)space {
    ...
    for (NSInteger i = 0; i < rows; i ++) {
        for (NSInteger j = 0; j < cols; j ++) {
            // 生成迷宫最小单元，保存到数据源中
            AttemptSinger *cell = [[AttemptSinger alloc] initWithRow:i withCol:j];
...
- (void)openMazeCellWithRow:(NSInteger)row WithCol:(NSInteger)col withDirection:(DIRECTION_TYPE)direction {
    // 获取指定迷宫单元
    AttemptSinger *cell = self.mazeCells[row * self.cols + col];
    // 开启指定方向
    [cell tinyStuff:direction];
}

#pragma mark 绘制迷宫图
- (DotClearHorizonView *)generateMazeView {
    
    CGFloat viewW = self.space * self.cols;
    CGFloat viewH = self.space * self.rows;
    
    DotClearHorizonView *mazeView = [[DotClearHorizonView alloc] initWithFrame:CGRectMake(0, 0, viewW, viewH)];
    ...
    // 生成足球，放在迷宫图上
    ConservativePrejudiceView *footballView = [self generateFootball];
...
#pragma mark 生成足球
- (ConservativePrejudiceView *)generateFootball {
    
    ConservativePrejudiceView *football = [[ConservativePrejudiceView alloc] initWithSpace:self.space];
...
- (void)bornMoveableGrocer {
    ...
    // 获取足球所在行列的cellModel
    AttemptSinger *cell = self.mazeCells[index];
    ...
    if (cell.canLeft && cell.col != 0) {
        [self.football starveAmongstAir];
    }
}
...
- (void)jailToEthos {
    ...
    if (cell.canUp && cell.row != 0) {
        [self.football wrestleAsIncident];
    }
}
...
- (void)castFurthermoreUtmostMorale {
    ...
    if (cell.canRight && cell.col != self.cols - 1) {
        [self.football purelySophisticatedPane];
        // 移动后判断是否已通过
...
- (void)followDeterminationFemale {
    ...
    if (cell.canDown && cell.row != self.rows - 1) {
        [self.football especiallyUnpleasantHorn];
